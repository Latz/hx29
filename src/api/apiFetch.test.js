...
describe("apiFetch", () => {
  ...
  it("refetches after cache TTL expires", async () => {
    ...
    global.fetch.mockResolvedValue(makeResponse([{ id: 2 }]));
  ...
  it("preserves X-WP-Total header through cache", async () => {
    global.fetch.mockResolvedValue(makeResponse([{ id: 1 }], 200, { "X-WP-Total": "42" }));
  ...
  it("preserves X-WP-TotalPages header through cache", async () => {
    global.fetch.mockResolvedValue(makeResponse([{ id: 1 }], 200, { "X-WP-TotalPages": "5" }));
  ...
  it("throws ApiError('server') on 5xx", async () => {
    global.fetch.mockResolvedValue(new Response("", { status: 500 }));
  ...
  it("clearApiCache forces fresh fetch", async () => {
    ...
    clearApiCache();
    global.fetch.mockResolvedValue(makeResponse([{ id: 99 }]));
